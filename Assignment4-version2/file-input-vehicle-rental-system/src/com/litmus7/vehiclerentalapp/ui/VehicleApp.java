...
public class VehicleApp {
	...
	public static void main(String[] args) {
		...
		if (loadResponse.getStatusCode() != 0) {
			...
			vehicles = (List<Vehicle>) loadResponse.getData();
		...
		while (true) {
			...
			case 1:
				...
				Response rentResponse = vehicleController.checkAvailability();
				if (rentResponse.getStatusCode() != 0) {
					...
					List<Vehicle> availableList = (List<Vehicle>) rentResponse.getData();
					System.out.println("\nChoose the number of the vehicle you want to rent:");
					int option = scanner.nextInt();
					scanner.nextLine(); // Clear buffer

					if (option > 0 && availableList.size() >= option) {
			...
			case 2:
				Car car = new Car("Honda", "City", 1300.0, true, 4, true);
				Response response = vehicleController.controlAddVehicle(car);
				...
				if (response.getStatusCode() != 0) {
					...
					List<Vehicle> updatedList = (List<Vehicle>) response.getData();
			...
			case 3:
				...
				Response searchResponse = vehicleController.controlSearch(searchKey);
				...
				if (searchResponse.getStatusCode() != 0) {
					...
					System.out.println("\nYour search list:");
					List<Vehicle> searchList = (List<Vehicle>) searchResponse.getData();
